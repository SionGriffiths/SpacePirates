...
function GreenLaserPulseMunition(originX, originY, targetDirection, aggressor, aggressorMomentum, turret){
	...
	this.animations = new Array();
	this.animations[0] = 0.05;
	this.animations[1] = 0.125;
	this.animations[2] = 0.175;
	this.animations[3] = 0.25;
	this.animations[4] = 0.375;
	this.animations[5] = 0.5;
	this.animationSize = 0.25;
	this.innerSize = 0.5;
	this.outerSize = 1.75;
	this.destroyed = false;
	...
	this.draw = function() {
		...
		if (!(this.destroyed)) {
			...
			c.fillStyle = gradient;
			c.fillRect(-20*z, -20*z, 40*z, 40*z);
			var gradient = c.createRadialGradient(0, 7*z, this.innerSize*z, 0, 7*z, this.outerSize*z);
